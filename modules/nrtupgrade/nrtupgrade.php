...
class NrtUpgrade extends Module implements WidgetInterface
{
    ...
    {			
        return parent::install() && $this->_createTab();		
    }
    ...
    {	
        if(Module::isEnabled('axoncreator')){
            ...
            $url = 'https://creator.axonvip.com/api_upgrade';
    ...
    public function downloadLast($dest, $filename = 'theme.zip')
    {
        ...
        try {
            ...
            $this->copy(self::$nrtLink, $destPath);
        }
    ...
    public static function nrtShouldUseFopen($url)
    {
    ...
    public static function nrt_file_get_contents($url, $use_include_path = false, $stream_context = null, $curl_timeout = 5)
    {
        ...
        if (self::nrtShouldUseFopen($url)) {
            ...
            if (isset($opts['http']['method']) && self::nrtStrtolower($opts['http']['method']) == 'post') {
