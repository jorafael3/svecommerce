...
class smartblog extends Module implements WidgetInterface
{
    ...
    public function install()
    {
        ...
        Configuration::updateGlobalValue('smartblogmetatitle', 'All Post');
        Configuration::updateGlobalValue('smartblogmetakeyword', 'axon,blog,smartblog,prestashop blog,prestashop,blog');
        Configuration::updateGlobalValue('smartblogmetadescrip', 'Prestashop powerfull blog site developing module. It has hundrade of extra plugins. This module developed by axonvip.com');
    ...
    {
		$this->BlogImageTypeCss($params);
		$this->BlogConfigCss($params);
    }
...
<p>The standard chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested. Sections 1.10.32 and 1.10.33 from \'de Finibus Bonorum et Malorum\' by Cicero are also reproduced in their exact original form, accompanied by English versions from the 1914 translation by H. Rackham.</p>';	
        ...
        for ($i = 1; $i <= 8; $i++) {
            Db::getInstance()->Execute('INSERT INTO `' . _DB_PREFIX_ . 'smart_blog_post_shop`(`id_smart_blog_post`, `id_shop`)	VALUES(' . $i . ',' . (int) $this->smart_shop_id . ')');
        }
        for ($i = 1; $i <= 7; $i++) {
            if ($i == 1):
    ...
    public function BlogConfigCss($params)
    {
        $id_shop = (int)$this->context->shop->id;
    ...
    private function _writeBlogImgCss()
    {
    ...
    public function BlogImageTypeCss($params)
    {
            ...
            if (ImageManager::validateUpload($FILES['avatar'], 4000000))
                ...
                if (!move_uploaded_file($FILES['avatar']['tmp_name'], $path))
                    ...
                    foreach ($author_types as $image_type) {
                        $dir = _PS_MODULE_DIR_ . 'smartblog/images/avatar/avatar_' . Tools::stripslashes($image_type['type_name']) . '.jpg';
                        if (file_exists($dir))
                            unlink($dir);
                    }
                    $images_types = BlogImageType::GetImageAllType('author');
                    foreach ($images_types as $image_type) {
                        ImageManager::resize($path, _PS_MODULE_DIR_ . 'smartblog/images/avatar/avatar_' . Tools::stripslashes($image_type['type_name']) . '.jpg', (int) $image_type['width'], (int) $image_type['height']
                        );
                    }
                }
            }
	...
	public function smartBlogRoutes()
    {
        ...
        $k_category = [];
        if( $pattern != 1 ){
            $k_category['id_category'] = array('regexp' => '[0-9]+', 'param' => 'id_category');
        }
        ...
        return array(
            ...
            'module-smartblog-search' => array(
				'controller' => 'search',
				'rule' => $slug .  '/' . Tools::link_rewrite($this->l('search')) . $html,
				'keywords' => array(),
				'params' => array(
					'fc' => 'module',
					'module' => 'smartblog'
            ...
            'module-smartblog-tagpost' => array(
				'controller' => 'tagpost',
				'rule' => $slug .  '/' . Tools::link_rewrite($this->l('tag')) . $html,
				'keywords' => array(),
				'params' => array(
					'fc' => 'module',
					'module' => 'smartblog'
            ...
            'module-smartblog-archive' => array(
                'controller' => 'archive',
				'rule' => $slug .  '/' . Tools::link_rewrite($this->l('archive')) . $html,
				'keywords' => array(),
                'params' => array(
                    'fc' => 'module',
                    'module' => 'smartblog'
            ...
            'module-smartblog-category' => array(
                'controller' => 'category',
                'rule' => $slug . '/' . Tools::link_rewrite($this->l('category')) . '/' . ( $pattern != 1 ? '{id_category}_' : '' ) . '{rewrite}' . $html,
                'keywords' => $k_category,
                'params' => array(
                    'fc' => 'module',
                    'module' => 'smartblog'
