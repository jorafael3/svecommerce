...
var SmartTree = function (element, options)
{
	this.$element = $(element);
	this.options = $.extend({}, $.fn.smartcattree.defaults, options);
	this.init();
};

SmartTree.prototype =
{
	constructor: SmartTree,
	...
	{            
			...
			function ()
			{
				...
				else
				{
					...
					if (load_tree)
					{
						...
						$.get(
							'ajax-tab.php',
							{controller:'AdminBlogPost',token:currentToken,action:'getCategoryTree',type:idTree,category:category,inputName:name,useCheckBox:useCheckBox},
							function(content)
							{
								thatOne.parent().closest('.tree-folder').find('ul.tree').html(content);
								$('#'+idTree).smartcattree('collapse', thatOne.closest('.tree-folder').children("ul.tree"));
								that.trigger('expand');
								thatOne.parent().parent().children("ul.tree").toggle(300);
								$('#'+idTree).smartcattree('init');
							}
						);
					}
					else
		...
		if (typeof(idTree) != 'undefined')
		{
			...
			if (typeof(SmarttreeClickFunc) != 'undefined')
			{
				this.$element.find(":input[type=radio]").unbind('click');
				this.$element.find(":input[type=radio]").click(SmarttreeClickFunc);
			}
		}
	...
	expandAll : function($speed)
	{
		...
		if (typeof(idTree) != 'undefined' && !$('#'+idTree).hasClass('full_loaded'))
		{
			...
			$.get(
				'ajax-tab.php',
				{controller:'AdminBlogPost',token:currentToken,action:'getCategoryTree',type:idTree,fullTree:1,selected:selected, inputName:name,useCheckBox:useCheckBox},
				function(content)
				{
					$('#'+idTree).html(content);
					$('#'+idTree).smartcattree('init');
...
$.fn.smartcattree = function (option, value)
{
	...
	var $set = this.each(
		function ()
		{
			...
			if (!data){
				$this.data('tree', (data = new SmartTree(this, options)));
			}
...
$.fn.smartcattree.Constructor = SmartTree;