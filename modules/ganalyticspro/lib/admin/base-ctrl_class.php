...
abstract class BT_GapBaseCtrl
{
    ...
    public static function get($sCtrlType, array $aParams = null)
    {
        ...
        // if valid controller
        if (file_exists(_GAP_PATH_LIB_ADMIN . $sCtrlType . '-ctrl_class.php')) {
            // require
            ...
            try {
                ...
                if ($oReflection->isInstantiable()) {
                    ...
                    throw new Exception(GAnalyticsPro::$oModule->l('Internal server error => controller isn\'t instantiable',
                        'base-ctrl_class'), 510);
                }
            } catch (ReflectionException $e) {
                throw new Exception(GAnalyticsPro::$oModule->l('Internal server error => invalid controller',
                    'base-ctrl_class'), 511);
            }
        } else {
            throw new Exception(GAnalyticsPro::$oModule->l('Internal server error => the controller file doesn\'t exist',
                'base-ctrl_class'), 512);
        }
    }
}