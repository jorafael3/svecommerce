{if $nrt_ppp.NRT_NEWSLETTER == 1}
    <div id="moda_popupnewsletter" class="modal popup-type-2 popup-text-color-{$nrt_ppp.NRT_NEWSLETTER_TEXT_COLOR}" tabindex="-1" role="dialog"><div class="modal-dialog" role="document" style="{if $nrt_ppp.NRT_BG == 1 && !empty($nrt_ppp.NRT_BG_IMAGE)}background-image: url({$nrt_ppp.NRT_BG_IMAGE});{/if}">
