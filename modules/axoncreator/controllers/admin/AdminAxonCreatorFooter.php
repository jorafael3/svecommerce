...
class AdminAxonCreatorFooterController extends ModuleAdminController
{
    public $name;
    ...
    public function __construct()
    {
            ...
            'general' => array(
                ...
                'fields' =>    array(
                    'active_footer_layout' => array(
                        ...
                        'cast' => 'intval',
                        'type' => 'select',
                        'list' => array_merge($list_pages, $this->module->getListByPostType('footer')),
                        'identifier' => 'id'
                    ),
                ),
		...
		$this->_where = ' AND `post_type` = "footer"';
        ...
        $this->name = 'AdminAxonCreatorFooter';
    }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submit' . $this->className)) {
			...
			if( count( $this->module->getListByPostType('footer') ) == 1 ){
				Configuration::updateValue( 'active_footer_layout', $returnObject->id );
			}
    ...
    public function renderForm()
    {
        ...
        if ($obj->id){
            $url = $this->context->link->getAdminLink('AxonCreatorEditor').'&post_type=footer&id_post=' . $obj->id . '&id_lang='. $id_lang;
        }
        else{
            $url = false;
        }
		
		$obj->post_type = 'footer';
