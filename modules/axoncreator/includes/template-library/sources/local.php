...
use AxonCreator\Core\Settings\Page\Model;
use AxonCreator\Modules\Library\Documents\Library_Document;
...
class Source_Local extends Source_Base {
	...
	public static function get_admin_url( $relative = false ) {
		...
		if ( ! $relative ) {
			$base_url = Wp_Helper::admin_url( $base_url );
		}
	...
	public function get_title() {
		return Wp_Helper::__( 'Local', 'elementor' );
	}
	...
	public function register_data() {
		$labels = [
			'name' => Wp_Helper::_x( 'My Templates', 'Template Library', 'elementor' ),
			'singular_name' => Wp_Helper::_x( 'Template', 'Template Library', 'elementor' ),
			'add_new' => Wp_Helper::_x( 'Add New', 'Template Library', 'elementor' ),
			'add_new_item' => Wp_Helper::_x( 'Add New Template', 'Template Library', 'elementor' ),
			'edit_item' => Wp_Helper::_x( 'Edit Template', 'Template Library', 'elementor' ),
			'new_item' => Wp_Helper::_x( 'New Template', 'Template Library', 'elementor' ),
			'all_items' => Wp_Helper::_x( 'All Templates', 'Template Library', 'elementor' ),
			'view_item' => Wp_Helper::_x( 'View Template', 'Template Library', 'elementor' ),
			'search_items' => Wp_Helper::_x( 'Search Template', 'Template Library', 'elementor' ),
			'not_found' => Wp_Helper::_x( 'No Templates found', 'Template Library', 'elementor' ),
			'not_found_in_trash' => Wp_Helper::_x( 'No Templates found in Trash', 'Template Library', 'elementor' ),
			'parent_item_colon' => '',
			'menu_name' => Wp_Helper::_x( 'Templates', 'Template Library', 'elementor' ),
		];
		 ...
		 */
		$args = Wp_Helper::apply_filters( 'elementor/template_library/sources/local/register_post_type_args', $args );
		...
		$args = [
			...
			'query_var' => Wp_Helper::is_admin(),
			'rewrite' => false,
			'public' => false,
			'label' => Wp_Helper::_x( 'Type', 'Template Library', 'elementor' ),
		];
		 ...
		 */
		$args = Wp_Helper::apply_filters( 'elementor/template_library/sources/local/register_taxonomy_args', $args );
		...
		$args = [
			...
			'public' => false,
			'labels' => [
				'name' => Wp_Helper::_x( 'Categories', 'Template Library', 'elementor' ),
				'singular_name' => Wp_Helper::_x( 'Category', 'Template Library', 'elementor' ),
				'all_items' => Wp_Helper::_x( 'All Categories', 'Template Library', 'elementor' ),
			],
		];
		 ...
		 */
		$args = Wp_Helper::apply_filters( 'elementor/template_library/sources/local/register_category_args', $args );
	}
	...
	public function admin_menu_reorder() {
		...
		if ( isset( $library_submenu[10][2] ) ) {
			...
			$library_submenu[700][2] = Wp_Helper::admin_url( self::ADMIN_MENU_SLUG . '#add_new' );
		}
	...
	public function admin_menu() {
		add_submenu_page( self::ADMIN_MENU_SLUG, '', Wp_Helper::__( 'Saved Templates', 'elementor' ), Editor::EDITING_CAPABILITY, self::get_admin_url( true ) );
	}
	...
	public function get_items( $args = [] ) {
		$templates_local = Wp_Helper::getTemplatesLocal();
		
		$templates = [];
				
		foreach ( $templates_local as $post ) {
			$templates[] = $this->get_item( $post['id_axon_creator_template'] );
		}
	...
	public function save_item( $template_data ) {
		...
		$defaults = [
			'title' => Wp_Helper::__( '(no title)', 'elementor' ),
			'page_settings' => [],
			'status' => 'publish',
		];
		
		$template_data = Wp_Helper::wp_parse_args( $template_data, $defaults );
		 ...
		 */
		Wp_Helper::do_action( 'elementor/template-library/after_save_template', $template_id, $template_data );
		 ...
		 * @param array $template_data The template data.
		 */
		Wp_Helper::do_action( 'elementor/template-library/after_update_template', $template_id, $template_data );
	...
	public function update_item( $new_data ) {
		...
		if ( ! $document ) {
			return new \WP_Error( 'save_error', Wp_Helper::__( 'Template not exist.', 'elementor' ) );
		}
		 ...
		 */
		Wp_Helper::do_action( 'elementor/template-library/after_update_template', $new_data['id'], $new_data );
	...
	public function get_item( $template_id ) {
		$post = new \AxonCreatorTemplate( $template_id );
		...
		$data = [
			...
			'url' => Wp_Helper::get_permalink_template( $post->id ),
		];
		 ...
		 */
		$data = Wp_Helper::apply_filters( 'elementor/template-library/get_template', $data );
	...
	public function get_data( array $args ) {
		...
		$template = new \AxonCreatorTemplate( $template_id );
	...
	public function export_template( $template_id ) {
		...
		if ( Wp_Helper::is_wp_error( $file_data ) ) {
			return $file_data;
		}
	...
	public function import_template( $name, $path ) {
		...
		if ( Wp_Helper::is_wp_error( $import_result ) ) {
	...
	public function post_row_actions( $actions, \WP_Post $post ) {
		if ( self::is_base_templates_screen() ) {
			if ( $this->is_template_supports_export( $post->ID ) ) {
				$actions['export-template'] = sprintf( '<a href="%1$s">%2$s</a>', $this->get_export_link( $post->ID ), Wp_Helper::__( 'Export Template', 'elementor' ) );
			}
		}
	...
	public function admin_import_template_form() {
		...
		<div id="elementor-hidden-area">
			<a id="elementor-import-template-trigger" class="page-title-action"><?php echo Wp_Helper::__( 'Import Templates', 'elementor' ); ?></a>
			<div id="elementor-import-template-area">
				<div id="elementor-import-template-title"><?php echo Wp_Helper::__( 'Choose an Elementor template JSON file or a .zip archive of Elementor templates, and add them to the list of templates available in your library.', 'elementor' ); ?></div>
				<form id="elementor-import-template-form" method="post" action="<?php echo Wp_Helper::admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					...
					<fieldset id="elementor-import-template-form-inputs">
						...
						<input type="submit" class="button" value="<?php echo Wp_Helper::esc_attr__( 'Import Now', 'elementor' ); ?>">
					</fieldset>
				</form>
			</div>
		</div>
	...
	public function block_template_frontend() {
		if ( Wp_Helper::is_singular( self::CPT ) && ! current_user_can( Editor::EDITING_CAPABILITY ) ) {
	...
	public function is_template_supports_export( $template_id ) {
		 ...
		 */
		$export_support = Wp_Helper::apply_filters( 'elementor/template_library/is_template_supports_export', $export_support, $template_id );
	...
	private function get_export_link( $template_id ) {
		...
		return Wp_Helper::get_ajax_editor( $params );
	}
	...
	private function save_item_type( $post_id, $type ) {
		Wp_Helper::update_post_meta( $post_id, Document::TYPE_META_KEY, $type );
	...
	public function admin_add_bulk_export_action( $actions ) {
		$actions[ self::BULK_EXPORT_ACTION ] = Wp_Helper::__( 'Export', 'elementor' );
	...
	public function admin_print_tabs( $views ) {
		...
		$baseurl = add_query_arg( $url_args, Wp_Helper::admin_url( 'edit.php' ) );
		...
		<div id="elementor-template-library-tabs-wrapper" class="nav-tab-wrapper">
			<a class="nav-tab<?php echo $active_class; ?>" href="<?php echo $baseurl; ?>">
				...
				if ( ! $all_title ) {
					$all_title = Wp_Helper::__( 'All', 'elementor' );
				}
				echo $all_title; ?>
			</a>
			<?php
		...
		<div class="elementor-template_library-blank_state">
			<div class="elementor-blank_state">
				...
				<h2>
					...
					printf( Wp_Helper::__( 'Create Your First %s', 'elementor' ), $current_type_label );
					?>
				</h2>
				<p><?php echo Wp_Helper::__( 'Add templates and reuse them across your website. Easily export and import them to any other project, for an optimized workflow.', 'elementor' ); ?></p>
				<a id="elementor-template-library-add-new" class="elementor-button elementor-button-success" href="<?php esc_url( Utils::get_pro_link( 'https://elementor.com/pro/?utm_source=wp-custom-fonts&utm_campaign=gopro&utm_medium=wp-dash' ) ); ?>">
					<?php
					/* translators: %s: Template type label. */
					printf( Wp_Helper::__( 'Add New %s', 'elementor' ), $current_type_label );
					?>
				</a>
			</div>
		</div>
	...
	public function add_filter_by_category( $post_type ) {
		...
		echo '<label class="screen-reader-text" for="cat">' . Wp_Helper::_x( 'Filter by category', 'Template Library', 'elementor' ) . '</label>';
	...
	private function prepare_template_export( $template_id ) {
		...
		return [
			'name' => 'axoncreator-' . $template_id . '-' . date( 'Y-m-d' ) . '.json',
	...
	private function get_template_label_by_type( $template_type ) {
		 ...
		 * @param string $template_type  Template type.
		 */
		$template_label = Wp_Helper::apply_filters( 'elementor/template-library/get_template_label_by_type', $template_label, $template_type );
	...
	private function add_actions() {
		Wp_Helper::add_action( 'template_redirect', [ $this, 'block_template_frontend' ] );
	}
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'elementor_library_type' => Wp_Helper::__( 'Type', 'elementor' ),
	...
	private function get_library_title() {
		...
		if ( $this->is_current_screen() ) {
			...
			if ( $current_tab_group ) {
				$titles = [
					'library' => Wp_Helper::__( 'Saved Templates', 'elementor' ),
					'theme' => Wp_Helper::__( 'Theme Builder', 'elementor' ),
					'popup' => Wp_Helper::__( 'Popups', 'elementor' ),
				];
