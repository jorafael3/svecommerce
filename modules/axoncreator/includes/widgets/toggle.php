...
class Widget_Toggle extends Widget_Base {
	...
	public function get_title() {
		return Wp_Helper::__( 'Toggle', 'elementor' );
	}
	...
	protected function _register_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'default' => Wp_Helper::__( 'Toggle Title', 'elementor' ),
				'label_block' => true,
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default' => Wp_Helper::__( 'Toggle Content', 'elementor' ),
				'show_label' => false,
			]
		);
		...
		$this->add_control(
			'tabs',
			[
				'label' => Wp_Helper::__( 'Toggle Items', 'elementor' ),
				'type' => Controls_Manager::REPEATER,
				'fields' => $repeater->get_controls(),
				'default' => [
					[
						'tab_title' => Wp_Helper::__( 'Toggle #1', 'elementor' ),
					...
					[
						'tab_title' => Wp_Helper::__( 'Toggle #2', 'elementor' ),
						'tab_content' => Wp_Helper::__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'elementor' ),
					],
				],
		...
		$this->add_control(
			...
			[
				...
				'default' => [
					'value' => 'fas fa-caret' . ( Wp_Helper::is_rtl() ? '-left' : '-right' ),
					'library' => 'fa-solid',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .elementor-toggle .elementor-tab-title .elementor-toggle-icon svg' => 'fill: {{VALUE}}; color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .elementor-toggle .elementor-tab-title.elementor-active .elementor-toggle-icon svg' => 'fill: {{VALUE}}; color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		if ( ! isset( $settings['icon'] ) && ! Icons_Manager::is_migration_allowed() ) {
			...
			// added as bc in 2.6
			// add old default
			$settings['icon'] = 'fa fa-caret' . ( Wp_Helper::is_rtl() ? '-left' : '-right' );
		...
		<div class="elementor-toggle" role="tablist">
			...
			foreach ( $settings['tabs'] as $index => $item ) :
				...
				<div class="elementor-toggle-item">
					<<?php echo Wp_Helper::esc_html( $settings['title_html_tag'] ); ?> <?php echo $this->get_render_attribute_string( $tab_title_setting_key ); ?>>
						<?php if ( $has_icon ) : ?>
						<span class="elementor-toggle-icon elementor-toggle-icon-<?php echo Wp_Helper::esc_attr( $settings['icon_align'] ); ?>" aria-hidden="true">
							<?php
							if ( $is_new || $migrated ) { ?>
							...
							<?php } else { ?>
								<i class="elementor-toggle-icon-closed <?php echo Wp_Helper::esc_attr( $settings['icon'] ); ?>"></i>
								<i class="elementor-toggle-icon-opened <?php echo Wp_Helper::esc_attr( $settings['icon_active'] ); ?>"></i>
							<?php } ?>
						</span>
						<?php endif; ?>
						<a href=""><?php echo $item['tab_title']; ?></a>
					</<?php echo Wp_Helper::esc_html( $settings['title_html_tag'] ); ?>>
