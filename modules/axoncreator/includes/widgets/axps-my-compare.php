 ...
 * @since 1.0.0
 */
class Widget_Axps_My_Compare extends Widget_Base {
	...
	public function get_title() {
		return Wp_Helper::__( 'My Compare', 'elementor' );
	}
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'top: {{SIZE}}{{UNIT}}'
					],
					'separator' => 'before',
				]
			);	
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'right: {{SIZE}}{{UNIT}}'
					],
					'separator' => 'before',
				]
			);	
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'min-width: {{SIZE}}{{UNIT}}; line-height: {{SIZE}}{{UNIT}}'
					],
					'separator' => 'before',
				]
			);	
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'font-size: {{SIZE}}{{UNIT}}'
					],
					'separator' => 'before',
				]
			);	
			...
			$this->add_control(
				...
				[
					...
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'fill: {{VALUE}}; color: {{VALUE}};',
					],
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .btn-canvas .compare-nbr' => 'background-color: {{VALUE}};',
					],
				]
			);
	...
	protected function render() {
		...
		if( \Module::isEnabled('nrtcompare') ) {
			...
			echo $this->fetch( 'module:nrtcompare/views/templates/hook/display-nb.tpl', [ 'icon' => $icon ] );
		}
