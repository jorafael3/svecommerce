...
class Settings_Controls {
	...
	private static function text( array $field ) {
		...
		?>
		<input type="<?php echo Wp_Helper::esc_attr( $field['type'] ); ?>" id="<?php echo Wp_Helper::esc_attr( $field['id'] ); ?>" name="<?php echo Wp_Helper::esc_attr( $field['id'] ); ?>" value="<?php echo Wp_Helper::esc_attr( Wp_Helper::get_option( $field['id'], $field['std'] ) ); ?>" <?php echo $attributes; ?>/>
		<?php
	...
	private static function checkbox( array $field ) {
		?>
		<label>
			<input type="<?php echo Wp_Helper::esc_attr( $field['type'] ); ?>" id="<?php echo Wp_Helper::esc_attr( $field['id'] ); ?>" name="<?php echo Wp_Helper::esc_attr( $field['id'] ); ?>" value="<?php echo $field['value']; ?>"<?php checked( $field['value'], Wp_Helper::get_option( $field['id'], $field['std'] ) ); ?> />
			<?php
			if ( ! empty( $field['sub_desc'] ) ) :
				echo $field['sub_desc'];
			endif;
	...
	private static function checkbox_list( array $field ) {
		$old_value = Wp_Helper::get_option( $field['id'], $field['std'] );
	...
	private static function select( array $field ) {
		$old_value = Wp_Helper::get_option( $field['id'], $field['std'] );
		?>
		<select name="<?php echo Wp_Helper::esc_attr( $field['id'] ); ?>">
			<?php if ( ! empty( $field['show_select'] ) ) : ?>
				<option value="">— <?php echo Wp_Helper::__( 'Select', 'elementor' ); ?> —</option>
			<?php endif; ?>

			<?php foreach ( $field['options'] as $value => $label ) : ?>
				<option value="<?php echo Wp_Helper::esc_attr( $value ); ?>"<?php selected( $value, $old_value ); ?>><?php echo $label; ?></option>
			<?php endforeach; ?>
		</select>
	...
	private static function checkbox_list_roles( array $field ) {
		...
		if ( is_multisite() ) {
			$roles = [
				'super_admin' => [
					'name' => Wp_Helper::__( 'Super Admin', 'elementor' ),
				],
			] + $roles;
		}
