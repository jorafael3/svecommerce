...
abstract class Settings_Page {
	...
	final public static function get_url() {
		return Wp_Helper::admin_url( 'admin.php?page=' . static::PAGE_ID );
	}
	...
	public function __construct() {
		if ( ! empty( $_POST['option_page'] ) && static::PAGE_ID === $_POST['option_page'] ) {
			Wp_Helper::add_action( 'admin_init', [ $this, 'register_settings_fields' ] );
		}
	}
	...
	private function ensure_tabs() {
		if ( null === $this->tabs ) {
			 ...
			 */
			Wp_Helper::do_action( "elementor/admin/after_create_settings/{$page_id}", $this );
		}
	}
}
