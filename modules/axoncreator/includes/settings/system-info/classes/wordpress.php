...
class WordPress_Reporter extends Base_Reporter {
	...
	public function get_memory_limit() {
		...
		if ( $memory_limit_bytes < $min_recommended_bytes ) {
			$result['recommendation'] = sprintf(
				...
				Wp_Helper::_x( 'We recommend setting memory to at least %1$s. For more information, read about <a href="%2$s">how to Increase memory allocated to PHP</a>.', 'System Info', 'elementor' ),
	...
	public function get_timezone() {
		$timezone = Wp_Helper::get_option( 'timezone_string' );
		if ( ! $timezone ) {
			$timezone = Wp_Helper::get_option( 'gmt_offset' );
		}
	...
	public function get_admin_email() {
		return [
			'value' => Wp_Helper::get_option( 'admin_email' ),
		];
	}
