...
namespace AxonCreator\System_Info;

use AxonCreator\System_Info\Classes\Abstracts\Base_Reporter;
use AxonCreator\System_Info\Helpers\Model_Helper;
...
class Main {
	...
	private function add_actions() {
		Wp_Helper::add_action( 'admin_menu', [ $this, 'register_menu' ], 500 );
		Wp_Helper::add_action( 'wp_ajax_elementor_system_info_download_file', [ $this, 'download_file' ] );
	}
	...
	public function display_page() {
		...
		<div id="elementor-system-info">
			<h3><?php echo Wp_Helper::__( 'System Info', 'elementor' ); ?></h3>
			<div><?php $this->print_report( $reports, 'html' ); ?></div>
			<h3><?php echo Wp_Helper::__( 'Copy & Paste Info', 'elementor' ); ?></h3>
			<div id="elementor-system-info-raw">
				<label id="elementor-system-info-raw-code-label" for="elementor-system-info-raw-code"><?php echo Wp_Helper::__( 'You can copy the below info as simple text with Ctrl+C / Ctrl+V:', 'elementor' ); ?></label>
			...
			<hr>
			<form action="<?php echo Wp_Helper::admin_url( 'admin-ajax.php' ); ?>" method="post">
				<input type="hidden" name="action" value="elementor_system_info_download_file">
				<input type="submit" class="button button-primary" value="<?php echo Wp_Helper::__( 'Download System Info', 'elementor' ); ?>">
			</form>
		</div>
		<?php
	}
	...
	public function download_file() {
		if ( ! current_user_can( $this->capability ) ) {
			wp_die( Wp_Helper::__( 'You don\'t have permissions to download this file', 'elementor' ) );
		}
	...
	public function register_menu() {
		$system_info_text = Wp_Helper::__( 'System Info', 'elementor' );
	...
	protected function get_default_settings() {
		...
		$base_lib_dir = AXON_CREATOR_PATH . 'includes/settings/system-info/';
