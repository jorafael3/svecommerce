...
class Tracker {
	...
	public static function init() {
		Wp_Helper::add_action( 'elementor/tracker/send_event', [ __CLASS__, 'send_tracking_data' ] );
		Wp_Helper::add_action( 'admin_init', [ __CLASS__, 'handle_tracker_actions' ] );
		Wp_Helper::add_action( 'admin_notices', [ __CLASS__, 'admin_notices' ] );
	}
	...
	public static function check_for_settings_optin( $new_value ) {
		$old_value = Wp_Helper::get_option( 'elementor_allow_tracking', 'no' );
	...
	public static function send_tracking_data( $override = false ) {
		 ...
		 */
		$override = Wp_Helper::apply_filters( 'elementor/tracker/send_override', $override );
		...
		if ( ! $override ) {
			 ...
			 */
			$last_send_interval = Wp_Helper::apply_filters( 'elementor/tracker/last_send_interval', $last_send_interval );
		...
		Wp_Helper::update_option( 'elementor_tracker_last_send', time() );

		// Send here..
		$params = [
			'system' => self::get_system_reports_data(),
			'site_lang' => get_bloginfo( 'language' ),
			'email' => Wp_Helper::get_option( 'admin_email' ),
			'usages' => [
		 ...
		 */
		$params = Wp_Helper::apply_filters( 'elementor/tracker/send_tracking_data_params', $params );

		Wp_Helper::add_filter( 'https_ssl_verify', '__return_false' );
	...
	public static function is_allow_track() {
		return 'yes' === Wp_Helper::get_option( 'elementor_allow_tracking', 'no' );
	}
	...
	public static function handle_tracker_actions() {
		...
		if ( 'opt_into' === $_GET['elementor_tracker'] ) {
			...
			Wp_Helper::update_option( 'elementor_allow_tracking', 'yes' );
			self::send_tracking_data( true );
		}
		...
		if ( 'opt_out' === $_GET['elementor_tracker'] ) {
			...
			Wp_Helper::update_option( 'elementor_allow_tracking', 'no' );
			Wp_Helper::update_option( 'elementor_tracker_notice', '1' );
		}
	...
	public static function admin_notices() {
		...
		if ( '1' === Wp_Helper::get_option( 'elementor_tracker_notice' ) ) {
			return;
		}
		...
		$tracker_description_text = Wp_Helper::__( 'Love using Elementor? Become a super contributor by opting in to our anonymous plugin data collection and to our updates. We guarantee no sensitive data is collected.', 'elementor' );
		 ...
		 */
		$tracker_description_text = Wp_Helper::apply_filters( 'elementor/tracker/admin_description_text', $tracker_description_text );
		?>
		<div class="notice updated elementor-message">
			<div class="elementor-message-inner">
				...
				<div class="elementor-message-content">
					<p><?php echo Wp_Helper::esc_html( $tracker_description_text ); ?> <a href="https://go.elementor.com/usage-data-tracking/" target="_blank"><?php echo Wp_Helper::__( 'Learn more.', 'elementor' ); ?></a></p>
					<p class="elementor-message-actions">
						<a href="<?php echo $optin_url; ?>" class="button button-primary"><?php echo Wp_Helper::__( 'Sure! I\'d love to help', 'elementor' ); ?></a>&nbsp;<a href="<?php echo $optout_url; ?>" class="button-secondary"><?php echo Wp_Helper::__( 'No thanks', 'elementor' ); ?></a>
					</p>
				</div>
			</div>
		</div>
	...
	private static function get_last_send_time() {
		$last_send_time = Wp_Helper::get_option( 'elementor_tracker_last_send', false );
		 ...
		 */
		$last_send_time = Wp_Helper::apply_filters( 'elementor/tracker/last_send_time', $last_send_time );
	...
	private static function get_elements_usage() {
		return Wp_Helper::get_option( Document::ELEMENTS_USAGE_OPTION_NAME );
