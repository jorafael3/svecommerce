...
abstract class Widget_Base extends Element_Base {
	...
	public function __construct( $data = [], $args = null ) {
		...
		if ( $is_type_instance ) {
			 ...
			 */
			Wp_Helper::do_action( "elementor/widget/{$widget_name}/skins_init", $this );
		}
	}
	...
	private function register_skin_control() {
		...
		if ( ! empty( $skins ) ) {
			...
			if ( $this->_has_template_content ) {
				$skin_options[''] = Wp_Helper::__( 'Default', 'elementor' );
			}
	...
	protected function parse_text_editor( $content ) {
		...
		$content = Wp_Helper::apply_filters( 'widget_text', $content, $this->get_settings() );
		return $content;
	}
	...
	public function render_content() {
		 ...
		 * @param Widget_Base $this The current widget.
		 */
		Wp_Helper::do_action( 'elementor/widget/before_render_content', $this );
		...
		<div class="elementor-widget-container">
			 ...
			 */
			$widget_content = Wp_Helper::apply_filters( 'elementor/widget/render_content', $widget_content, $this );
	...
	protected function deprecated_notice( $plugin_title, $since, $last = '', $replacement = '' ) {
		$this->start_controls_section( 'Deprecated',
			[
				'label' => Wp_Helper::__( 'Deprecated', 'elementor' ),
			]
		);
