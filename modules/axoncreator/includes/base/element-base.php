...
abstract class Element_Base extends Controls_Stack {
	...
	final public function enqueue_scripts() {
		foreach ( $this->get_script_depends() as $script ) {
			Wp_Helper::wp_enqueue_script( $script );
		}
	}
	...
	final public function enqueue_styles() {
		foreach ( $this->get_style_depends() as $style ) {
			Wp_Helper::wp_enqueue_style( $style );
		}
	}
	...
	final public static function is_edit_buttons_enabled() {
		return Wp_Helper::get_option( 'elementor_edit_buttons' );
	}
	...
	public function get_help_url() {
		return 'https://creator.axonvip.com/widget-' . $this->get_name();
	}
	...
	public function add_link_attributes( $element, array $url_control, $overwrite = false ) {
		...
		if ( ! empty( $url_control['url'] ) ) {
			$allowed_protocols = array_merge( Wp_Helper::wp_allowed_protocols(), [ 'skype', 'viber' ] );

			$attributes['href'] = Wp_Helper::esc_url( $url_control['url'], $allowed_protocols );
		}
	...
	public function print_element() {
		 ...
		 */
		Wp_Helper::do_action( 'elementor/frontend/before_render', $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		Wp_Helper::do_action( "elementor/frontend/{$element_type}/before_render", $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		$should_render = Wp_Helper::apply_filters( "elementor/frontend/{$element_type}/should_render", $should_render, $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		Wp_Helper::do_action( "elementor/frontend/{$element_type}/after_render", $this );
		 ...
		 * @param Element_Base $this The element.
		 */
		Wp_Helper::do_action( 'elementor/frontend/after_render', $this );
	}
	...
	protected function _add_render_attributes() {
		...
		if ( ! empty( $settings['_attributes'] ) ) {
			...
			if ( null === $black_list ) {
				 ...
				 */
				$black_list = Wp_Helper::apply_filters( 'elementor_pro/element/attributes/black_list', $black_list );
			}
		 ...
		 * @param Element_Base $this The element.
		 */
		Wp_Helper::do_action( 'elementor/element/after_add_attributes', $this );
	}
	...
	private function get_child_type( $element_data ) {
		 ...
		 */
		$child_type = Wp_Helper::apply_filters( 'elementor/element/get_child_type', $child_type, $element_data, $this );
