...
abstract class Manager {
	...
	public function __construct() {
		Wp_Helper::add_action( 'elementor/editor/init', [ $this, 'on_elementor_editor_init' ] );

		Wp_Helper::add_action( 'elementor/ajax/register_actions', [ $this, 'register_ajax_actions' ] );

		$name = $this->get_css_file_name();

		Wp_Helper::add_action( "elementor/css-file/{$name}/parse", [ $this, 'add_settings_css_rules' ] );
		
		Wp_Helper::add_action( 'elementor/css-file/post/parse', [ $this, 'add_page_settings_css' ] );
	}
	...
	final public function ajax_save_settings( $request ) {
		 ...
		 * @param array $data                  Settings data.
		 */
		$success_response_data = Wp_Helper::apply_filters( "elementor/settings/{$settings_name}/success_response_data", $success_response_data, $id, $data );
	...
	protected function print_editor_template_content( $name ) {
		...
		<div class="elementor-panel-navigation">
			<# _.each( elementor.config.settings.<?php echo Wp_Helper::esc_html( $name ); ?>.tabs, function( tabTitle, tabSlug ) { #>
				<div class="elementor-panel-navigation-tab elementor-tab-control-{{ tabSlug }}" data-tab="{{ tabSlug }}">
					<a href="#">{{{ tabTitle }}}</a>
				</div>
				<# } ); #>
		</div>
		<div id="elementor-panel-<?php echo Wp_Helper::esc_attr( $name ); ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-elementor-panel-<?php echo Wp_Helper::esc_attr( $name ); ?>-settings">
