...
class Manager extends BaseManager {
		...
		foreach ( $model_controls as $tab_name => $sections ) {
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					$saved_setting = Wp_Helper::get_option( $control_name, null );

					if ( null !== $saved_setting ) {
						$settings[ $control_name ] = Wp_Helper::get_option( $control_name );
					}
				}
			}
		}
	...
	protected function save_settings_to_db( array $settings, $id ) {
			...
			foreach ( $sections as $section_name => $section_data ) {
				...
				foreach ( $section_data['controls'] as $control_name => $control_data ) {
					if ( isset( $settings[ $control_name ] ) ) {
						...
						Wp_Helper::update_option( $control_name, $settings[ $control_name ] );
					} else {
						Wp_Helper::delete_option( $control_name );
					}
				}
			}
		}
		...
		if ( ! empty( $one_list_settings ) ) {
			Wp_Helper::update_option( self::META_KEY, $one_list_settings );
		} else {
			Wp_Helper::delete_option( self::META_KEY );
		}
	}
	...
	private function add_panel_tabs() {
		Controls_Manager::add_tab( self::PANEL_TAB_LIGHTBOX, Wp_Helper::__( 'Lightbox', 'elementor' ) );
	}
}
