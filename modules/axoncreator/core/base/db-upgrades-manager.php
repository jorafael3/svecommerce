...
abstract class DB_Upgrades_Manager extends Background_Task_Manager {
	...
	public function get_task_runner_class() {
		return 'AxonCreator\Core\Upgrade\Updater';
	}
	...
	public function get_current_version() {
		if ( null === $this->current_version ) {
			$this->current_version = Wp_Helper::get_option( $this->get_version_option_name() );
		}
	...
	public function on_runner_start() {
		...
		define( 'IS_AXON_CREATOR_UPGRADE', true );
	}
	...
	public function admin_notice_start_upgrade() {
		...
		$message = '<p>' . sprintf( Wp_Helper::__( '%s Your site database needs to be updated to the latest version.', 'elementor' ), $this->get_updater_label() ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, Wp_Helper::__( 'Update Now', 'elementor' ) ) . '</p>';
	...
	public function admin_notice_upgrade_is_running() {
		...
		$message = '<p>' . sprintf( Wp_Helper::__( '%s Database update process is running in the background.', 'elementor' ), $this->get_updater_label() ) . '</p>';
	...
	public function admin_notice_upgrade_is_completed() {
		...
		$message = '<p>' . sprintf( Wp_Helper::__( '%s The database update process is now complete. Thank you for updating to the latest version!', 'elementor' ), $this->get_updater_label() ) . '</p>';
	...
	protected function update_db_version() {
		Wp_Helper::update_option( $this->get_version_option_name(), $this->get_new_version() );
	}
	...
	public function __construct() {
		...
		if ( $updater->is_running() ) {
			Wp_Helper::add_action( 'admin_notices', [ $this, 'admin_notice_upgrade_is_running' ] );
		}
