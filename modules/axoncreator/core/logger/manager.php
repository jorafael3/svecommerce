...
use AxonCreator\Core\Common\Modules\Ajax\Module;
use AxonCreator\Core\Logger\Loggers\Logger_Interface;
use AxonCreator\Core\Logger\Items\PHP;
use AxonCreator\Core\Logger\Items\JS;
use AxonCreator\Plugin;
use AxonCreator\System_Info\Main as System_Info;
...
class Manager extends BaseModule {
	...
	public function shutdown() {
		...
		$elementor_path = untrailingslashit( wp_normalize_path( AXON_CREATOR_PATH ) );
	...
	private function register_loggers() {
		if ( ! did_action( 'elementor/loggers/register' ) ) {
			Wp_Helper::do_action( 'elementor/loggers/register', $this );
		}
	}
	...
	public function __construct() {
		...
		Wp_Helper::add_action( 'admin_init', [ $this, 'add_system_info_report' ] );

		Wp_Helper::add_action( 'wp_ajax_elementor_js_log', [ $this, 'js_log' ] );

		Wp_Helper::add_action( 'elementor/loggers/register', [ $this, 'register_default_loggers' ] );
	}
}
