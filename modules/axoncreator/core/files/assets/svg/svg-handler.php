...
namespace AxonCreator\Core\Files\Assets\Svg;
...
class Svg_Handler {
	...
	public static function is_svg_uploads_enabled() {
		return ! ! Wp_Helper::get_option( 'elementor_allow_svg', false );
	}
	...
	protected function get_meta() {
		return Wp_Helper::get_post_meta( $this->attachment_id, self::META_KEY, true );
	}
	...
	protected function update_meta( $meta ) {
		Wp_Helper::update_post_meta( $this->attachment_id, self::META_KEY, $meta );
	}
	...
	protected function delete_meta() {
		Wp_Helper::delete_post_meta( $this->attachment_id, self::META_KEY );
	}
	...
	public function delete_meta_cache() {
		Wp_Helper::delete_post_meta_by_key( self::META_KEY );
	}
	...
	public function read_from_file() {
		return \Tools::file_get_contents( get_attached_file( $this->attachment_id ) );
	}
	 ...
	 */
	public static function get_inline_svg( $url ) {

		$svg = \Tools::file_get_contents( $url );
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( 'svg' !== $ext ) {
			$file['error'] = sprintf( Wp_Helper::__( 'The uploaded %s file is not supported. Please upload a valid svg file', 'elementor' ), $ext );
			return $file;
		}

		if ( ! self::is_enabled() ) {
			$file['error'] = Wp_Helper::__( 'SVG file is not allowed for security reasons', 'elementor' );
			return $file;
		}

		if ( self::svg_sanitizer_can_run() && ! $this->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = Wp_Helper::__( 'Invalid SVG Format, file not uploaded for security reasons', 'elementor' );
		}
	...
	public function sanitize_svg( $filename ) {
		$original_content = \Tools::file_get_contents( $filename );
	...
	private function get_allowed_attributes() {
		...
		return Wp_Helper::apply_filters( 'elementor/files/svg/allowed_attributes', $allowed_attributes );
	}
	...
	private function get_allowed_elements() {
		...
		return Wp_Helper::apply_filters( 'elementor/files/svg/allowed_elements', $allowed_elements );
	}
	...
	public static function is_enabled() {
		...
		if ( null === $enabled ) {
			...
			$enabled = Wp_Helper::apply_filters( 'elementor/files/svg/enabled', $enabled );
		}
		return $enabled;
	}
	...
	public function __construct() {
		Wp_Helper::add_filter( 'upload_mimes', [ $this, 'upload_mimes' ] );
		Wp_Helper::add_filter( 'wp_handle_upload_prefilter', [ $this, 'wp_handle_upload_prefilter' ] );
		Wp_Helper::add_filter( 'wp_check_filetype_and_ext', [ $this, 'wp_check_filetype_and_ext' ], 10, 4 );
		Wp_Helper::add_filter( 'wp_prepare_attachment_for_js', [ $this, 'wp_prepare_attachment_for_js' ], 10, 3 );
		Wp_Helper::add_action( 'elementor/core/files/clear_cache', [ $this, 'delete_meta_cache' ] );
	}
}
