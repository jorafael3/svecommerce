...
use AxonCreator\Core\Common\Modules\Connect\Admin;
...
abstract class Base_App {
	...
	const SITE_URL = 'https://creator.axonvip.com/connect/';

	const API_URL = 'https://creator.axonvip.com/connect/';
	...
	public function admin_notice() {
		...
		echo '</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">' .
			Wp_Helper::__( 'Dismiss', 'elementor' ) .
			'</span></button></div>';
	...
	public function action_get_token() {
		...
		$this->add_notice( Wp_Helper::__( 'Connected Successfully.', 'elementor' ) );
	...
	public function action_disconnect() {
		if ( $this->is_connected() ) {
			$this->disconnect();
			$this->add_notice( Wp_Helper::__( 'Disconnected Successfully.', 'elementor' ) );
		}
	...
	protected function request( $action, $request_body = [] ) {
		...
		$response = Wp_Helper::wp_remote_post( $this->get_api_url() . '/' . $action, [
			'body' => $request_body,
			'headers' => $headers,
			'timeout' => 25,
		] );
	...
	protected function get_site_key() {
		$site_key = Wp_Helper::get_option( 'elementor_connect_site_key' );

		if ( ! $site_key ) {
			$site_key = md5( uniqid( wp_generate_password() ) );
			Wp_Helper::update_option( 'elementor_connect_site_key', $site_key );
		}
	...
	public function __construct() {
		Wp_Helper::add_action( 'admin_notices', [ $this, 'admin_notice' ] );
