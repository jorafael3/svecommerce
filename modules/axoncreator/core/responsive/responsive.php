...
namespace AxonCreator\Core\Responsive;
...
class Responsive {
		...
		return array_reduce(
			array_keys( self::$default_breakpoints ), function( $new_array, $breakpoint_key ) {
				if ( ! in_array( $breakpoint_key, self::$editable_breakpoints_keys ) ) {
					...
					$saved_option = Wp_Helper::get_option( self::BREAKPOINT_OPTION_PREFIX . $breakpoint_key );
	...
	public static function get_stylesheet_templates_path() {
		return AXON_CREATOR_ASSETS_PATH . 'css/templates/';
	}
	...
	private static function get_stylesheet_templates() {
		...
		return Wp_Helper::apply_filters( 'elementor/core/responsive/get_stylesheet_templates', $templates );
	}
}
