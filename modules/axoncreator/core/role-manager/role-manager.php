...
namespace AxonCreator\Core\RoleManager;

use AxonCreator\Settings_Page;
...
class Role_Manager extends Settings_Page {
	...
	public function get_role_manager_options() {
		return Wp_Helper::get_option( 'elementor_' . self::ROLE_MANAGER_OPTION_NAME, [] );
	}
	...
	protected function get_page_title() {
		return Wp_Helper::__( 'Role Manager', 'elementor' );
	}
			...
			'general' => [
				...
				'sections' => [
					'tools' => [
						'fields' => [
							'exclude_user_roles' => [
								'label' => Wp_Helper::__( 'Exclude Roles', 'elementor' ),
								'field_args' => [
	...
	public function display_settings_page() {
		...
		?>
		<div class="wrap">
			<h1><?php echo Wp_Helper::esc_html( $this->get_page_title() ); ?></h1>

			<div id="elementor-role-manager">
				<h3><?php echo Wp_Helper::__( 'Manage What Your Users Can Edit In Elementor', 'elementor' ); ?></h3>
	...
	private function display_role_controls( $role_slug, $role_data ) {
		...
		?>
		<div class="elementor-role-row <?php echo Wp_Helper::esc_attr( $role_slug ); ?>">
			<div class="elementor-role-label">
				<span class="elementor-role-name"><?php echo Wp_Helper::esc_html( $role_data['name'] ); ?></span>
				<span data-excluded-label="<?php Wp_Helper::esc_attr_e( 'Role Excluded', 'elementor' ); ?>" class="elementor-role-excluded-indicator"></span>
			...
			<div class="elementor-role-controls hidden">
				<div class="elementor-role-control">
					<label>
						<input type="checkbox" name="elementor_exclude_user_roles[]" value="<?php echo Wp_Helper::esc_attr( $role_slug ); ?>"<?php checked( in_array( $role_slug, $excluded_options, true ), true ); ?>>
						<?php echo Wp_Helper::__( 'No access to editor', 'elementor' ); ?>
					</label>
				</div>
				<div>
					<?php
					 ...
					 * @param array  $role_data An array with role data.
					 */
					Wp_Helper::do_action( 'elementor/role/restrictions/controls', $role_slug, $role_data );
					?>
				</div>
			</div>
		</div>
	...
	public function get_go_pro_link_html() {
		$pro_link = Utils::get_pro_link( 'https://creator.axonvip.com/pro/?utm_source=wp-role-manager&utm_campaign=gopro&utm_medium=wp-dash' );
		?>
		<div class="elementor-role-go-pro">
			<div class="elementor-role-go-pro__desc"><?php echo Wp_Helper::__( 'Want to give access only to content?', 'elementor' ); ?></div>
			<div class="elementor-role-go-pro__link"><a class="elementor-button elementor-button-default elementor-button-go-pro" target="_blank" href="<?php echo esc_url( $pro_link ); ?>"><?php echo Wp_Helper::__( 'Go Pro', 'elementor' ); ?></a></div>
		</div>
		<?php
	}
	...
	private function get_user_restrictions() {
		...
		if ( ! $restrictions ) {
			 ...
			 */
			$restrictions = Wp_Helper::apply_filters( 'elementor/editor/user/restrictions', $restrictions );
		}
		return $restrictions;
	}
	...
	public function __construct() {
		...
		Wp_Helper::add_action( 'admin_menu', [ $this, 'register_admin_menu' ], 100 );
		Wp_Helper::add_action( 'elementor/role/restrictions/controls', [ $this, 'get_go_pro_link_html' ] );
	}
}
