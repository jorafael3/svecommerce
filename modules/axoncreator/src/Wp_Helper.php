...
define( 'AXON_CREATOR_VERSION', '1.2.4' );
define( 'AXON_CREATOR_PREVIOUS_STABLE_VERSION', '1.2.3' );
...
class Wp_Helper {
	...
	const STORE_URL = 'https://creator.axonvip.com/api_licenses';
	const RENEW_URL = 'https://creator.axonvip.com/api_renew';
		...
		switch ( $post_type ) {
            case 'hook':
					if ( $key_related == 'displayProductSummary' || $key_related == 'displayFooterProduct' || $key_related == 'displayLeftColumnProduct' || $key_related == 'displayRightColumnProduct' || $key_related == 'displayProductAccessories' || $key_related == 'displayProductSameCategory' ) {
                        ...
                        $permalink = $context->link->getProductLink( $products[0]['id_product'], null, null, null, $id_lang, $id_shop );
					...
					}elseif( $key_related == 'displayLeftColumn' || $key_related == 'displayRightColumn' || $key_related == 'displayFooterCategory' ){
    ...
    {
        return Tools::getAdminToken('AxonCreatorEditor'.(is_object(Context::getContext()->employee) ? (int)Context::getContext()->employee->id :
