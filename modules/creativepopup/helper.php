...
function cp_get_posts($params = null)
{
    ...
    $sql = 'SELECT DISTINCT p.id_product, p.date_add, p.date_upd,
        pl.name, pl.link_rewrite, pl.description, pl.description_short, m.name AS manufacturer
        ...
        ($order_by == 'quantity' ? ' LEFT JOIN `'._DB_PREFIX_.'product_sale` ps
            ON ps.id_product = p.id_product' : '').
        ($order_by == 'reduction' ? ' INNER JOIN `'._DB_PREFIX_.'specific_price` sp
        ...
        WHERE pl.`id_lang` = '.(int)$id_lang.
            ...
            ($order_by == 'reduction'
                ? ' AND sp.from <= NOW() AND (sp.to >= NOW() OR sp.to = "0000-00-00 00:00:00")'
                : ''
            ).'
...
{
    $user = new Employee((int) $userid);
    $user->user_nicename = $user->firstname.' '.$user->lastname;
    return $user;
}

function cp_get_avatar_url($userid)
{
    $user = new Employee((int) $userid);
    return method_exists($user, 'getImage')
        ? $user->getImage()
...
function cp_add_user_meta($user_id, $key, $value)
{
    return cp_add_option('u'.(int)$user_id.'_'.$key, $value);
}

function cp_get_user_meta($user_id, $key = '')
{
    return cp_get_option('u'.(int)$user_id.'_'.$key);
}

function cp_update_user_meta($user_id, $key, $value)
{
...
function cp_die($message = '', $title = '')
{
    die($title ? "$title \n$message" : $message);
}

function cp__($text, $domain = 'creativepopup')
{
    return cp_ss(Translate::getModuleTranslation($domain, $text, '', null, true));
}

function cp_e($text, $domain = 'creativepopup')
{
    echo cp_ss(Translate::getModuleTranslation($domain, $text, '', null, true));
}

function cp_x($text, $context, $domain = 'creativepopup')
{
    // TODO
    $context .= '';
    return cp_ss(Translate::getModuleTranslation($domain, $text, '', null, true));
...
function cp_ex($text, $context, $domain = 'creativepopup')
{
    // TODO
    $context .= '';
    echo cp_ss(Translate::getModuleTranslation($domain, $text, '', null, true));
...
function cp_n($single, $plural, $number, $domain = 'creativepopup')
{
    return cp_ss(Translate::getModuleTranslation($domain, $number > 1 ? $plural : $single, '', null, true));
}

function cp_get_allowed_mime_types()
{
    return cp_apply_filters('allowed_mime_types', cp_get_mime_types());
}
...
function cp_get_index()
{
    $table = _DB_PREFIX_.'creativepopup_revisions';
    $data = Db::getInstance()->getValue("SELECT data FROM $table WHERE id = -1");
    return json_decode(empty($data) ? '[]' : $data, true);
}

function cp_update_index($data)
{
    return Db::getInstance()->insert(
        'creativepopup_revisions',
        array(
            'id' => -1,
            'popup_id' => 0,
            'author' => 0,
...
{
    return Configuration::updateValue(cp_get_option_key($option), Tools::jsonEncode($value));
}
...
{
    $GLOBALS['cp_style'][$handle] = array('src' => $src, 'deps' => $deps, 'ver' => $ver, 'media' => $media);
}

$GLOBALS['cp_script'] = array();

function cp_enqueue_script($handle, $src = false, $deps = array(), $ver = false)
{
    if (!empty($deps) && 0) {
        throw new Exception('TODO');
    }
...
function cp_register_script($handle, $src, $deps = array(), $ver = false)
{
    $GLOBALS['cp_script'][$handle] = array(
        'src' => $src,
        'deps' => $deps,
        'ver' => $ver,
        'priority' => CP_SCRIPTS_PRIORITY
    );
}
...
function cp_localize_script($handle, $name, $data)
{
    if (version_compare(_PS_VERSION_, '1.6.0.11', '>=')) {
        ...
        $GLOBALS['ls_local'][$handle] = "var $name = ".Tools::jsonEncode($data).';';
    }
}
...
require_once _PS_MODULE_DIR_.'creativepopup/base/classes/CpCache.phtml';

function cp_set_transient($transient, $value, $expiration = 0)
{
    $expiration = (int) $expiration;
    try {
        $result = call_user_func('CpCache::getInstance')->set($transient, $value, $expiration);
    } catch (Exception $ex) {
        $result = false;
    }
    return $result;
...
{
    try {
        $result = call_user_func('CpCache::getInstance')->get($transient);
    } catch (Exception $ex) {
        $result = false;
    }
    return $result;
...
{
    try {
        $result = call_user_func('CpCache::getInstance')->delete($transient);
    } catch (Exception $ex) {
        $result = false;
    }
    return $result;
...
function cp_get_image_type_name($name)
{
...
{
    $context = Context::getContext();
    if (!empty($context->cookie->cp_key)) {
        die(json_encode(array('key' => $context->cookie->cp_key, 'success' => 1)));
    }
    if (empty($context->cookie->username_addons) || empty($context->cookie->password_addons)) {
        ...
        $btn = ' <a href="'.$context->link->getAdminLink('AdminModules').
            '" style="float:none; display:inline-block">'.cp__('HERE').'</a>';
    ...
    if ($res = Tools::file_get_contents('https://api.addons.prestashop.com', false, $streamContext)) {
        $xml = simplexml_load_string($res);
        foreach ($xml->module as $mod) {
            if ($mod->name == 'creativepopup') {
                $context->cookie->cp_key = md5((string)$mod->id);
    ...
    $btn = ' <a href="https://addons.prestashop.com/pop-up/39348-creative-popup.html" '.
        'target="_blank" style="float:none; display:inline-block">'.cp__('HERE').'</a>';
    die(Tools::jsonEncode(array('msg' => $msg.$btn, 'success' => 0)));
}
...
function cp_download_popup()
{
    ...
    if ($data) {
        if (file_put_contents($destination, $data)) {
            ...
            Tools::redirectAdmin(
                $context->link->getAdminLink('AdminCreativePopup').
                '&action=edit&id='.$import->lastImportId.'#appearance'
            );
        } else {
            $context->cookie->cp_error = cp__('Unable to write file: ').$destination;
        }
    } else {
        $context->cookie->cp_error = cp__('Tools::file_get_contents returned without data!');
    }
...
function cp_get_media_link($filepath)
{
...
function cp_pre_parse_defaults($data)
{
    if (!empty($data['properties']['backgroundimage']) && $data['properties']['backgroundimage'][0] == '/') {
        $data['properties']['backgroundimage'] = cp_get_media_link($data['properties']['backgroundimage']);
    }
    foreach ($data['layers'] as &$page) {
        if (!empty($page['properties']['background']) && $page['properties']['background'][0] == '/') {
            $page['properties']['background'] = cp_get_media_link($page['properties']['background']);
        }
        if (!empty($page['properties']['thumbnail']) && $page['properties']['thumbnail'][0] == '/') {
            $page['properties']['thumbnail'] = cp_get_media_link($page['properties']['thumbnail']);
        }
        if (!empty($page['sublayers'])) {
            foreach ($page['sublayers'] as &$layer) {
                if (!empty($layer['image']) && $layer['image'][0] == '/') {
                    $layer['image'] = cp_get_media_link($layer['image']);
                }
                if (!empty($layer['poster']) && $layer['poster'][0] == '/') {
                    $layer['poster'] = cp_get_media_link($layer['poster']);
                }
            }
        }
    }
