...
function cp_init_admin_scripts()
{
    ...
    $mediamanagerurl = preg_replace('~^https?:~', '', $context->link->getAdminLink('AdminCreativePopupMedia'));
    $adminmodulesurl = preg_replace(
        '~^https?:~',
        '',
        $context->link->getAdminLink('AdminModules').'&configure=creativepopup&module_name=creativepopup'
    );
    $ajaxurl = 'index.php?controller=AdminCreativePopup&ajax=1&token='.CP_URL_TOKEN;
    $cpVersion = CP_PLUGIN_VERSION;
    $cpSaveHistory = cp_get_option('cp_save_history', false) ? 1 : 0;
