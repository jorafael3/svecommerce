...
// Context menu
CP_contextMenuTop = 10,
CP_contextMenuLeft = 10,
...
var CreativePopup = {
	...
	addFormattedLayer: function( el, layerProperties ) {
		...
		jQuery('.cp-overlay.dim').remove();
		jQuery('.cp-layer-types').animate({ marginTop : 40, opacity : 0 }, 150, function() {
			jQuery(this).hide();
		});

		// Get default layer data
		var layerData = jQuery.extend(true, {}, CP_DataSource.getDefaultLayerData() ),
			layerType = jQuery(el).data('type');
		...
		switch( layerType ) {
			...
			case 'text':
				layerData.html = CP_l10n.SBPreviewTextPlaceholder;
				break;

			case 'html':
				layerData.html = CP_l10n.SBPreviewHTMLPlaceholder;
				break;

			case 'button':
				layerData.html = CP_l10n.SBPreviewButtonPlaceholder;
				jQuery.extend( layerData.styles, {
					'padding-top': 8,
					'padding-right': 20,
					'padding-bottom': 8,
			...
			case 'post':
				layerData.html = CP_l10n.SBPreviewPostPlaceholder;
				break;
		...
		// Add formatted layer
		CreativePopup.addLayer( layerData, null, {
			selectLayer: true,
			selectPage: 0
		});
	},
...
checkForOpeningTransition: function() {
	...
	removeLayer: function(layerIndexSet, removeProperties) {
		...
		if( ! CP_activeSlideData.sublayers.length ) {
			jQuery('.cp-timeline-switch, .cp-sublayer-nav').hide();
			jQuery('.cp-multi-select-notice').hide();
			jQuery('.cp-sublayer-pages').empty();
	...
	hideLayer: function( el ) {
		...
		var layerIndexSet 	= CP_activeLayerIndexSet,
			layerDataSet 	= CP_activeLayerDataSet,
			updateInfo 		= [],
			layerData,
			$control;
		...
		if( el ) {
			...
			layerDataSet 	= [ CP_activeSlideData.sublayers[ layerIndexSet[0] ] ];
		}
		...
		jQuery.each( layerIndexSet, function( index, layerIndex ) {
			...
			$control 	= jQuery('.cp-sublayers .cp-icon-eye').eq(layerIndex);
		...
		// Maintain history
		CP_UndoManager.add('layer.general', CP_l10n.SBUndoHideLayer, updateInfo);
	},
	...
	lockLayer: function(el) {
		...
		jQuery.each( layerIndexSet, function( index, layerIndex ) {
			...
			$control 		= jQuery('.cp-sublayers .cp-icon-lock').eq(layerIndex);
	...
	selectMediaType: function(el, layerIndex) {
		...
		switch(section) {
			...
			case 'button':
				...
				jQuery('.cp-html-code .cp-options, .cp-html-code .cp-insert-media', layer).addClass('cp-hidden');
				jQuery('.cp-html-code textarea').attr('placeholder', placeholder );
				jQuery('.cp-sublayer-thumb', $layerItem).attr('class', 'cp-sublayer-thumb dashicons dashicons-marker').html('');
				break;
	...
	generatePreviewItem: function(layerIndex, post, generateProperties) {
		...
		if(type == 'img') {
			...
			var tmpContent = url ? '<img src="'+url+'">' : '<div class="cp-layer-placeholder cp-image-placeholder"><span class="dashicons dashicons-format-image"></span><span>'+CP_l10n.SBPreviewImagePlaceholder+'</span></div>';
			...
			if( layerData.media === 'media' && html === '' ) {
				item = jQuery('<div class="cp-layer-placeholder cp-media-placeholder"><span class="dashicons dashicons-video-alt3"></span><span>'+CP_l10n.SBPreviewMediaPlaceholder+'</span></div>').appendToWithIndex(generateProperties.$targetArea, layerIndex);
	...
	updatePreviewSelection: function() {
		...
		if( CP_activeLayerIndexSet.length === 1 ) {
			var layerIndex = CP_activeLayerIndexSet[0];

			if( CP_previewItems[layerIndex] ) {
				if( CP_previewItems[layerIndex].hasClass('cp-layer-placeholder') ) {
	...
	contextMenu: function(e) {
		...
		if( CreativePopup.isSlidePreviewActive || CreativePopup.isLayerPreviewActive || document.location.href.indexOf('cp-revisions') !== -1 ) {
			return;
		}
		...
		CP_contextMenuTop = e.pageY - CP_previewArea.offset().top;
		CP_contextMenuLeft = e.pageX - CP_previewArea.offset().left;
		...
		jQuery('.cp-preview-context-menu').remove();

		// Create list
		var $list = jQuery( jQuery('#tmpl-cp-preview-context-menu').text() ).prependTo('body');
			$list.hide().css({ top: mt, left: ml }).fadeIn(100);

		// Close event
		jQuery('body').on('click.cp-context-menu', function() {
			jQuery('body').unbind('click.cp-context-menu');
			jQuery('.cp-preview-context-menu').animate({ opacity: 0 }, 200, function() {
				jQuery(this).remove();
			});
		});
		...
		if(items.length > 1) {
			jQuery.each(items, function(idx, data) {
				...
				var layerIndex = data.index,
					...
					$li = jQuery('<li><p></p><span>'+layerData.subtitle+'</span></li>').appendTo( $list.find('.cp-context-overlapping-layers ul') );
					$li.data('layerIndex', layerIndex);

				CreativePopup.setLayerMedia( layerData.media,  jQuery('p', $li), layerData );
			});
		} else {
			$list.find('.cp-context-overlapping-layers').hide();
		}
	...
	editLayerStart: function( $layer ) {
		...
		if( $layer.is('img') || CP_activeLayerDataSet[0].media === 'img' ) {
			jQuery('.cp-layer-image').click();
			return false;

		} else if( CP_activeLayerDataSet[0].media === 'media' ) {
			return false;
		}
	...
	startSlidePreview: function( sliderOptions ) {
		...
		}).on('popupDidLoad', CreativePopup.handleSubmitOnPreview);
	},
	...
	handleSubmitOnPreview: function(e, slider) {
		// show notice that submit isn't allowed on preview
		slider.slider.on('click', 'button, input[type=submit]', function() {
			kmUI.notify.show({
				icon: 'dashicons-info',
				iconColor: '#ffffff',
				text: CP_l10n.notifySubmitDisabled
			});
	...
	startPopupPreview: function( sliderOptions, button ) {
		...
		if (jQuery('#cp-popup-preview').creativePopup('closePopup').length) {
			return;
		}

		if (!sessionStorage['cpNotifyEscPopup']) {
			sessionStorage['cpNotifyEscPopup'] = 1;

			kmUI.notify.show({
				icon: 'dashicons-info',
				iconColor: '#ffffff',
				text: CP_l10n.notifyEscPopup
			});
			setTimeout(function() {
				kmUI.notify.hide();
			}, 2000);
		...
		$slider.one('popupDidClose', function() {
			...
			popupDisableOverlay: sliderProps.popupDisableOverlay,
			popupOverlayClickToClose: sliderProps.popupOverlayClickToClose,
	...
	populateSliderPreview: function( $slider, plugins, sliderData ) {
		...
		if( callbacks ) {
			for( var key in callbacks ) {
				...
				$slider.on(key, new Function('event', 'popup', body));
			}
		}
	},
			...
			success: function(jqXHR, textStatus) {
				try {
					...
					if (~jqXHR.indexOf('name="submitLogin"')) {
						...
						win.history.pushState('', '', location.origin+location.pathname+'?controller=AdminLogin&redirect=AdminCreativePopup');
					}
				}
...
var initSliderBuilder = function() {
	...
	// Select layer type
	}).on('click', '.cp-layer-types li', function() {
		CreativePopup.addFormattedLayer( this );

	}).on('click', '.cp-context-add-layer li', function() {
		CreativePopup.addFormattedLayer( this, {
			styles: {
				top: CP_contextMenuTop / CP_previewZoom,
				left: CP_contextMenuLeft / CP_previewZoom
			}
		});

	}).on('click', '.cp-context-menu-duplicate', function() {
		CreativePopup.duplicateLayer();

	}).on('click', '.cp-context-menu-remove', function() {
		CreativePopup.removeLayer();

	}).on('click', '.cp-context-menu-hide', function() {
		CreativePopup.hideLayer();

	}).on('click', '.cp-context-menu-lock', function() {
		CreativePopup.lockLayer();

	}).on('click', '.cp-context-menu-copy-styles', function() {
		CreativePopup.copyLayerSettings( jQuery('.cp-sublayer-style .cp-h-actions .copy') );

	}).on('click', '.cp-context-menu-paste-styles', function() {
		CreativePopup.pasteLayerSettings( jQuery('.cp-sublayer-style .cp-h-actions .paste') );

	}).on('click', '.cp-context-menu-copy-layer', function() {
		CreativePopup.copyLayer( );

	}).on('click', '.cp-context-menu-paste-layer', function() {
	...
	jQuery('#cp-layers').on('click', '.cp-add-sublayer', function(e) {
		...
		jQuery('.cp-layer-types').show().animate({ marginTop : 15, opacity : 1 }, 150);
		jQuery('<div>', { 'class' : 'cp-overlay dim'}).insertAfter('.cp-layer-types');
	...
	}).on('click', '.cp-overlay.dim', function() {
	...
	CP_previewWrapper.on('contextmenu',function(e) {
		e.preventDefault(); CreativePopup.contextMenu(e);
	});
	...
	jQuery(document).on({
		...
		}, '.cp-context-overlapping-layers li'
	);

	// Select layer from intersecting layers list
	jQuery(document).on('click', '.cp-context-overlapping-layers li', function(event) {
	...
	jQuery(document).on('keydown', function(e) {
		...
		if( e.which == 27 ) {
			e.preventDefault();
			return jQuery('#cp-popup-preview').creativePopup('closePopup');
		}
