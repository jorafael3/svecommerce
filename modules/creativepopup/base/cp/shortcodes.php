...
class CpShortcode
{
    ...
    public static function generateErrorMarkup(
        $title = null,
        $description = null,
        $logo = 'dashicons-warning',
        $customClass = ''
    ) {
        if (! $title) {
        ...
        if (is_null($description)) {
            $description = cp__(
                "Please make sure that you've used the right method to insert the popup, ".
                "and check if the corresponding popup exists and it wasn't deleted previously."
            );
        }
