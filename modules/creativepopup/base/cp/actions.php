...
function cp_register_form_actions()
{
    ...
    if (!empty(Context::getContext()->employee->id)) {
        // Popups list layout
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'layout'
        ) {
        ...
        // Duplicate popup
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'duplicate'
        ) {
        ...
        // Empty caches
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'empty_caches'
        ) {
        ...
        // Update Library
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'update_store'
        ) {
        ...
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'hide-important-notice'
        ) {
        ...
        if (isset(${'_GET'}['page']) && ${'_GET'}['page'] == 'popups' &&
            isset(${'_GET'}['action']) && ${'_GET'}['action'] == 'hide-support-notice'
        ) {
...
{
    try {
        call_user_func('CpCache::getInstance')->delete('cp-popup-data-*');
    } catch (Exception $ex) {
        // TODO
    }
}
