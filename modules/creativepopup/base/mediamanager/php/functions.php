...
function cp_display_gallery_page($files_array, $pageno = 1, $path = '', $resultspp = 4096, $display = true)
{
    ...
    foreach ($files_array as $file_name) {
        ...
        if (($pagination['counter'] >= $pagination['startres']) && ($pagination['counter'] <= $pagination['endres'])) {
            ...
            if ($pagination['counter'] > ($pagination['endres'] - 5) &&
                $pagination['counter'] > ($pagination['totalres'] - 5)
            ) {
                $addcbr = ' br';
            }
...
function cp_get_gallery_html($pconfig/*, $ext = ''*/)
{
    ...
    if ($current_page < $round_pages) {
        ...
        if ($tot_pages > 1) {
            $links_html .= '<li id="gliLast" class="disabled">'.
                '<a href="#" data-target-page="$round_pages">Last &gt;</a><li>';
        }
    }
    //if ($round_pages > 9) {}
    $links_html .= '</ul>';
