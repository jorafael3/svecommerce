...
$pages['properties']['props']['width'] = !empty($pages['properties']['attrs']['popupWidth'])
    ? $pages['properties']['attrs']['popupWidth']
    : 640
;
$pages['properties']['props']['height'] = !empty($pages['properties']['attrs']['popupHeight'])
    ? $pages['properties']['attrs']['popupHeight']
    : 360
;
...
$cpContainer[] = '<form id="'.$popupID.'" name="'.$popupID.
    '" class="cp-ps-container fitvidsignore" method="post" action="'.$_SERVER['REQUEST_URI'].
    '" style="'.implode('', $popupStyleAttr).'">';
...
if (!empty($popup['slides']) && is_array($popup['slides'])) {
    foreach ($popup['slides'] as $pagekey => $page) {
        ...
        if (!empty($pages['properties']['props']['post_tax_terms'])) {
            $queryArgs['img_size'] = $pages['properties']['props']['post_tax_terms'];
        }
        ...
        if (! empty($page['props']['background'])) {
            ...
            if (! empty($page['props']['backgroundId'])) {
                $cpBG = cp_get_attachment_image(
                    $page['props']['backgroundId'],
                    'full',
                    false,
                    array('class' => 'cp-bg')
                );
        ...
        // Add page thumbnail
        if (!isset($pages['properties']['attrs']['thumbnailNavigation']) ||
            $pages['properties']['attrs']['thumbnailNavigation'] != 'disabled'
        ) {
            if (!empty($page['props']['thumbnail'])) {
                $src = !empty($page['props']['thumbnailId'])
                    ? cp_apply_filters('cp_get_image', $page['props']['thumbnailId'], $page['props']['thumbnail'])
                    : $page['props']['thumbnail']
                ;
                $cpMarkup[] = '<img src="'.$src.'" class="cp-tn" alt="Page thumbnail" />';
            } elseif (empty($page['props']['background'])) {
                $skin = isset($pages['properties']['attrs']['skin'])
        ...
        if (!empty($page['layers']) && is_array($page['layers'])) {
            foreach ($page['layers'] as $layerkey => $layer) {
                ...
                if (!empty($layer['props']['media']) && $layer['props']['media'] == 'post') {
                    $layer['props']['post_text_length'] = !empty($layer['props']['post_text_length'])
                        ? $layer['props']['post_text_length']
                        : 0
                    ;
                    $layer['props']['html'] = $postContent->getWithFormat(
                        $layer['props']['html'],
                        $layer['props']['post_text_length']
                    );
                }
                ...
                if ($layer['props']['type'] == 'img') {
                    if (! empty($layer['props']['imageId'])) {
                        $layerIMG = cp_get_attachment_image(
                            (int)$layer['props']['imageId'],
                            'full',
                            false,
                            array('class' => 'cp-l')
                        );
                ...
                $video = $inner->find(
                    ...
                    'iframe[src*="youtu.be"], iframe[src*="player.vimeo"]'
                );
                if ($video->length) {
