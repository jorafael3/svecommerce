    ...
    protected static function autoinclude()
    {
        ...
        foreach (self::$index as &$popup) {
            ...
            if (!empty($popup['shop']) && $popup['shop'] != $context->shop->id ||
                ...
                $subscribed > 0 &&
                ($customer && empty($customer->newsletter) || !$customer && empty(${'_COOKIE'}['cp-subscribed'])) ||
                $subscribed < 0 &&
                ($customer && !empty($customer->newsletter) || !$customer && !empty(${'_COOKIE'}['cp-subscribed']))
            ) {
                continue;
            ...
            if (!$popup['repeat'] && !empty(${'_COOKIE'}['cp-popup-'.$popup['id']])) {
                ...
                if (!(float)$popup['repeat_days']) {
                    if (!empty(${'_COOKIE'}['cp-popup-'.$popup['id']])) {
                        continue;
                    }
                } else {
                    if (!empty(${'_COOKIE'}['cp-popup-'.$popup['id']]) &&
                        ${'_COOKIE'}['cp-popup-'.$popup['id']] > $time - 86400 * (float)$popup['repeat_days']
                    ) {
                        continue;
                    }
                }
            ...
            if (!in_array('0', $popup['roles']) && !count(array_intersect($groups, $popup['roles'])) ||
                ...
                (empty($popup['urls']) || !preg_match($popup['urls'], $url))
            ) {
                continue;
            }
