...
class CpImportUtil
{
    ...
    public function uploadMedia($dir = null)
    {
        ...
        foreach (glob($dir.'/uploads/*') as $filePath) {
            ...
            if (!empty($filetype['ext']) && $filetype['ext'] != 'php') {
                // New upload
                if (! $attach_id = $this->attachIDForURL($targetFile)) {
                    // Move item to place
    ...
    public function attachIDForURL($path)
    {
        ...
        $attachs = $cpdb->getCol(
            $cpdb->prepare("SELECT ID FROM {$cpdb->prefix}posts WHERE guid RLIKE %s;", $imgPath[1])
        );
