...
class CpInstances
{
    ...
    public static function find($args = array())
    {
        ...
        // Find by query
            ...
            $popups = $cpdb->getResults(
                "SELECT SQL_CALC_FOUND_ROWS {$args['columns']} FROM $table $where
                ORDER BY {$args['orderby']} {$args['order']} LIMIT {$args['limit']}",
                ARRAY_A
            );
    ...
    private static function getByIds($ids = null)
    {
        // Check ID
        if (!is_array($ids)) {
            return false;
    ...
    public static function changeStatus($id, $published)
    {
        if (is_array($id)) {
            ...
            $res = Db::getInstance()->update(
                CP_DB_TABLE,
                array('flag_hidden' => $published ? 0 : 1),
                "id IN ($ids)",
                count($id)
            );
        } else {
        ...
        if ($res) {
            $res &= $published ? CpPopups::addIndex($id) : CpPopups::removeIndex($id);
        }
        return $res;
    }
}
