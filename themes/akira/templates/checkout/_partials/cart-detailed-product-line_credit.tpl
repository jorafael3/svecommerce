...
<td class="product-c-price" data-title="{l s='Price' d='Shop.Theme.Axon'}">
	<div class="amount {if $product.has_discount}has-discount{/if}">
		{assign var="price_" value=str_replace("$", "", $product.price)}
		<span>{Tools::displayPrice(($price_ * 1.16 ) /12)}</span>
		{if $product.unit_price_full}
			{assign var="price_2" value=str_replace("$", "", $product.unit_price_full)}
			<div class="unit-price-cart">{Tools::displayPrice(($price_2 * 1.16 ) /12)}</div>
		{/if}
	</div>
